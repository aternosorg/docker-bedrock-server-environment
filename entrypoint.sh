...
if [ "$NEEDS_LEGACY_1_0" = true ]; then
    ...
    if [ "$IS_ARM" = true ]; then
        ...
        box64 "$SERVER_BIN" 2>&1 | tee -a server.log
    else
        LD_LIBRARY_PATH=".:/usr/lib/legacy-x86_64:$LD_LIBRARY_PATH" "$SERVER_BIN" 2>&1 | tee -a server.log
    fi
    ...
    if [ "$IS_ARM" = true ]; then
        ...
        LD_LIBRARY_PATH=".:$LD_LIBRARY_PATH" "$SERVER_BIN" 2>&1 | tee -a server.log
    fi
fi